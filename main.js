  ...
  if (s1 === SENHA_1 && s2 === SENHA_2) {
    ...
    if (vidas > 0) {
      ...
      setTimeout(() => {
        ...
        document.getElementById('fase1').style.display = ''; // <-- aqui!
...
window.pularFase = function(fase) {
  ...
  document.getElementById('resultado').innerHTML = '';
  ['feedback1','feedback2','feedback3','feedback4','qc-feedback'].forEach(id => {
    const el = document.getElementById(id);
  ...
  if (fase === 1) {
    document.getElementById('fase1').style.display = '';
  } else if (fase === 2) {
    document.getElementById('fase2').style.display = '';
  } else if (fase === 3) {
    document.getElementById('fase3').style.display = '';
  } else if (fase === 4) {
...
window.verificarFase1 = function() {
  const senha = document.getElementById('senha-fase1').value.trim().toLowerCase();
  const feedback = document.getElementById('feedback-fase1');
  if (senha === "proxima fase") {
    feedback.textContent = "Correto!";
    feedback.className = "correto";
    setTimeout(() => {
      mostrarFase('fase2');
      // Foca no input da fase 2, se existir
      const inputFase2 = document.getElementById('input-fase2');
      if (inputFase2) inputFase2.focus();
    }, 700);
  } else {
    feedback.textContent = "Tente novamente!";
    feedback.className = "incorreto";
...
const fraseFase2 = "eu te amo";
const barraFase2 = document.getElementById('barra-fase2');
barraFase2.innerHTML = '';
for (let i = 0; i < fraseFase2.length; i++) {
  const div = document.createElement('div');
  div.className = 'barra-letra';
  div.id = 'letra-fase2-' + i;
  barraFase2.appendChild(div);
}
let posFase2 = 0;
document.getElementById('input-fase2').value = '';
document.getElementById('input-fase2').addEventListener('input', function(e) {
  const letra = e.target.value;
  if (letra === fraseFase2[posFase2]) {
    document.getElementById('letra-fase2-' + posFase2).classList.add('preenchido');
    // Mostra o texto digitado na caixa
    let textoAtual = '';
    for (let i = 0; i < posFase2; i++) textoAtual += fraseFase2[i];
    textoAtual += letra;
    e.target.value = textoAtual;
    posFase2++;
    setTimeout(() => { e.target.value = ''; }, 100); // Limpa para próxima letra
    if (posFase2 === fraseFase2.length) {
      document.getElementById('feedback-fase2').textContent = "Correto!";
      document.getElementById('feedback-fase2').className = "correto";
      setTimeout(() => {
        mostrarFase('fase3');
        posFase2 = 0;
        for (let i = 0; i < fraseFase2.length; i++) {
          document.getElementById('letra-fase2-' + i).classList.remove('preenchido');
        }
      }, 700);
    }
  } else {
...
window.verificarFase3 = function() {
  const resposta = document.getElementById('resposta-fase3').value.trim();
  const feedback = document.getElementById('feedback-fase3');
  if (resposta === "7") { // Troque para o número correto da sua flor!
    feedback.textContent = "Correto! Parabéns!";
    feedback.className = "correto";
    setTimeout(() => {
      mostrarFase('pergunta1');
    }, 900);
  } else {
    feedback.textContent = "Tente novamente!";
    feedback.className = "incorreto";
...
function mostrarFase(faseId) {
  // Lista todos os IDs das fases e do formulário de senha
  const ids = [
    'fase1', 'fase2', 'fase3',
    'pergunta1', 'pergunta2', 'pergunta3', 'pergunta4', 'pergunta5',
    'senha-form'
  ...
  // Mostra apenas a fase desejada
  const proxima = document.getElementById(faseId);
  if (proxima) proxima.style.display = (proxima.tagName === 'FORM') ? 'block' : '';
}

const imgNormal = document.getElementById('img-frase-normal');
const imgSombra = document.getElementById('img-frase-sombra');
imgNormal.onmouseover = () => { imgNormal.style.display = 'none'; imgSombra.style.display = ''; };
imgSombra.onmouseout = () => { imgNormal.style.display = ''; imgSombra.style.display = 'none'; };